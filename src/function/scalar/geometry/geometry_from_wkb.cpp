...
void GeometryFromWKB::RegisterFunction(BuiltinFunctions &set) {
	...
	from_text.AddFunction(ScalarFunction({LogicalType::VARCHAR}, LogicalType::GEOMETRY, GeometryFromWKBFunction));
	from_text.AddFunction(
	    ScalarFunction({LogicalType::VARCHAR, LogicalType::INTEGER}, LogicalType::GEOMETRY, GeometryFromWKBFunction));
